		...
		(function () {
			...
			e.extend(d.node.prototype, {
				appendTo: function (h, i) {
					...
					while (h) {
						if (h.type == 1) {
							if (h.is('body') || h.getCustomData('_cke_notReadOnly'))
								break;
			...
			e.extend(h.prototype, {
				type: 1, addClass: function (i) {
					...
					return function () {
						var j = this;
						if (!(f.$removeEmpty[j.getName()] || j.is('a')))
							return;
						i(j, j.getNext(), true);
						i(j, j.getPrevious());
					};
			...
			e.extend(a.editor.prototype, {
				addCommand: function (l, m) {
					...
					if (!l)
						r.updateElement();
					if (r.mode)
						r._.modes[r.mode].unload(r.getThemeSpace('contents'));
					r.theme.destroy(r);
					var m, n = 0, o, p, q;
					if (r.toolbox) {
						...
						for (; n < m.length; n++) {
							...
							for (o = 0; o < p.length; o++) {
								q = p[o];
								if (q.clickFn)
									e.removeFunction(q.clickFn);
								if (q.keyDownFn)
									e.removeFunction(q.keyDownFn);
								if (q.index)
									k.button._.instances[q.index] = null;
							}
						}
					}
					if (r.contextMenu)
						e.removeFunction(r.contextMenu._.functionId);
					if (r._.filebrowserFn)
						e.removeFunction(r._.filebrowserFn);
					r.fire('destroy');
					a.remove(r);
			...
			(function () {
				var l = function (t) {
					...
					for (var M = E; M < C.length; M++) {
						...
						K = I.getNext();
						while (K) {
							if (K.equals(D[M]) || K.equals(x))
								break;
							L = K.getNext();
							if (u == 2)
								H.append(K.clone(true));
							else {
								K.remove();
								if (u == 1)
									H.append(K);
					...
					for (var N = E; N < D.length; N++) {
						...
						if (!C[N] || I.$.parentNode != C[N].$.parentNode) {
							K = I.getPrevious();
							while (K) {
								if (K.equals(C[N]) || K.equals(w))
									break;
								L = K.getPrevious();
								if (u == 2)
									H.$.insertBefore(K.$.cloneNode(true), H.$.firstChild);
								else {
									K.remove();
									if (u == 1)
					...
					if (u == 2) {
						...
						if (F && G && (w.$.parentNode != F.$.parentNode || x.$.parentNode != G.$.parentNode)) {
							var Q = G.getIndex();
							if (A && G.$.parentNode == w.$.parentNode)
								Q--;
			...
			(function () {
				...
				function u(w, x) {
					...
					if (w.checkBoundaryOfElement(y, 1) && w.checkBoundaryOfElement(y, 2)) {
						var z;
						while (y && y.type == 1 && (z = y.getParent()) && z.getChildCount() == 1 && (!(y.getName() in x) || z.getName() in x))
							y = z;
			...
			(function () {
				...
				o.prototype = {
					exec: function (t, u) {
						...
						var v = n(t, this.type);
						if (!v)
							alert(t.lang.clipboard[this.type + 'Error']);
						return v;
					}
				};
				var p = {
					canUndo: false, exec: c ? function (t) {
						...
						try {
							if (!t.document.getBody().fire('beforepaste') && !t.document.$.execCommand('Paste', false, null))
								throw 0;
						} catch (u) {
							setTimeout(function () {
								t.fire('pasteDialog');
				...
				j.add('clipboard', {
					requires: ['dialog', 'htmldataprocessor'], init: function (t) {
						...
						function u(y, z, A, B) {
							...
							if (t.addMenuItems)
								t.addMenuItem(z, {label: C, command: z, group: 'clipboard', order: B});
						}
						;
				...
				function n() {
					...
					if (o) {
						...
						if (q == 'dir') {
							...
							if (s != r && o.getParent())
								this._.dialog._.editor.fire('dirChanged', {
									node: o,
			...
			(function () {
				...
				function q(u) {
					...
					if (x)
						return p.call(D, v, 2);
					if (!D.useIndentClasses && D.name == 'indent')
						return p.call(D, v, 2);
					...
					if (D.useIndentClasses) {
						...
						if (D.name == 'outdent' && !B || D.name == 'indent' && B == v.config.indentClasses.length)
							return p.call(D, v, 0);
						return p.call(D, v, 2);
					} else {
				...
				function r(u, v) {
					...
					if (x.useIndentClasses) {
						...
						x.indentClassMap = {};
						for (var w = 0; w < u.config.indentClasses.length; w++)
							x.indentClassMap[u.config.indentClasses[w]] = w + 1;
					}
				...
				r.prototype = {
					exec: function (u) {
						...
						function x(N) {
							...
							while (O && !O.getParent().equals(N))
								O = O.getParent();
							while (P && !P.getParent().equals(N))
								P = P.getParent();
							if (!O || !P)
								return;
							var Q = O, R = [], S = false;
							...
							for (U = X.getCustomData('listarray_index') + 1; U < Y.length && Y[U].indent > Z; U++)
								Y[U].indent += V;
							...
							if (v.name == 'outdent') {
								...
								if ((ac = N.getParent()) && ac.is('li')) {
									...
									for (U = af - 1; U >= 0; U--) {
										if ((ag = ad.getItem(U)) && ag.is && ag.is('li'))
											ae.push(ag);
									}
								}
							}
							if (ab)
								ab.listNode.replace(N);
							if (ae && ae.length)
						...
						function z(N, O) {
							...
							if (v.useIndentClasses) {
								...
								if (Q > 0)
									N.addClass(u.config.indentClasses[Q - 1]);
							} else {
						...
						while (D = E.getNextRange()) {
							...
							if (G) {
								...
								if (!(M && (v.name == 'indent' || v.useIndentClasses || parseInt(G.getStyle(s(G)), 10)) && z(G, !K && J.getDirection())))
									x(G);
							} else
								y();
						}
				...
				j.add('indent', {
					init: function (u) {
						...
						if (b.ie6Compat || b.ie7Compat)
							u.addCss('ul,ol{\tmargin-left: 0px;\tpadding-left: 40px;}');
			...
			j.add('link', {
				init: function (m) {
					...
					m.on('selectionChange', function (n) {
						...
						if (p && p.getName() == 'a' && p.getAttribute('href'))
							o.setState(2);
						else
							o.setState(0);
					});
					m.on('doubleclick', function (n) {
						...
						if (!o.isReadOnly())
							if (o.is('a'))
								n.data.dialog = o.getAttribute('name') && !o.getAttribute('href') ? 'anchor' : 'link';
							else if (o.is('img') && o.data('cke-real-element-type') == 'anchor')
								n.data.dialog = 'anchor';
					});
					if (m.addMenuItems)
						m.addMenuItems({
			...
			(function () {
				...
				j.list = {
					listToArray: function (F, G, H, I, J) {
						...
						for (var K = 0, L = F.getChildCount(); K < L; K++) {
							var M = F.getChild(K);
							if (M.$.nodeName.toLowerCase() != 'li')
								continue;
							var N = {parent: F, indent: I, element: M, contents: []};
							if (!J) {
								N.grandparent = F.getParent();
								if (N.grandparent && N.grandparent.$.nodeName.toLowerCase() == 'li')
									N.grandparent = N.grandparent.getParent();
							} else
								N.grandparent = J;
							if (G)
								h.setMarker(G, M, 'listarray_index', H.length);
							H.push(N);
							for (var O = 0, P = M.getChildCount(), Q; O < P; O++) {
								Q = M.getChild(O);
								if (Q.type == 1 && m[Q.getName()])
									j.list.listToArray(Q, G, H, I + 1, N.grandparent);
								else
									N.contents.push(Q);
							}
						}
						...
						while (1) {
							...
							if (R.indent == O) {
								...
								for (var S = 0; S < R.contents.length; S++)
									P.append(R.contents[S].clone(1, 1));
								N++;
								...
								if (m[R.grandparent.getName()])
									...
									if (!J && I == 2 && !P.hasAttributes())
										P = new d.documentFragment(K);
								} else
									P = new d.documentFragment(K);
								...
								if (P.type == 11 && N != F.length - 1) {
									...
									if (U && U.type == 1 && U.getAttribute('type') == '_moz')
										U.remove();
									if (!(U = P.getLast(q) && U.type == 1 && U.getName() in f.$block))
										P.append(K.createElement('br'));
								}
								...
								return null;
							if (F.length <= N || Math.max(F[N].indent, 0) < O)
								break;
						}
						if (G) {
							var Y = L.getFirst();
				...
				function s(F) {
					...
					for (var K = 0; K < I.length && (J = I[K]) && !J.equals(H); K++) {
						if (m[I[K].getName()])
							return r.call(this, F.editor, this.type == I[K].getName() ? 1 : 2);
					}
				...
				function v(F, G, H) {
					...
					for (var N = 0; N < I.length; N++)
						M = M.getCommonAncestor(I[N].getParent());
				...
				function w(F, G, H) {
					...
					function U(V) {
						if ((S = R[V ? 'getFirst' : 'getLast']()) && !(S.is && S.isBlockBoundary()) && (T = G.root[V ? 'getPrevious' : 'getNext'](d.walker.whitespaces(true))) && !(T.is && T.isBlockBoundary({br: 1})))
					...
					exec: function (F) {
						...
						if (this.state == 2) {
							...
							if (!J.getFirst()) {
								...
								if (K.is('br')) {
									...
									I[0].selectNodeContents(K);
								H.selectRanges(I);
							} else {
								var L = I.length == 1 && I[0], M = L && L.getEnclosedNode();
								if (M && M.is && this.type == M.getName())
									r.call(this, F, 1);
							}
						}
						...
						while ((L = Q.getNextRange()) && ++R) {
							...
							if (T.type == 1 && T.getName() == 'td')
								L.setStartAt(S.startNode, 1);
							if (U.type == 1 && U.getName() == 'td')
								L.setEndAt(S.endNode, 2);
							var V = L.createIterator(), W;
							V.forceBrBreak = this.state == 2;
							while (W = V.getNextParagraph()) {
								if (W.getCustomData('list_block'))
									continue;
								else
									h.setMarker(P, W, 'list_block', 1);
								...
								if (ab)
									continue;
								var ag = ac;
								if (ag.getCustomData('list_group_object_' + R))
									ag.getCustomData('list_group_object_' + R).contents.push(W);
								else {
						...
						while (O.length > 0) {
							af = O.shift();
							if (this.state == 2) {
								if (m[af.root.getName()])
									t.call(this, F, af, P, ah);
								else
									v.call(this, F, af, ah);
							} else if (this.state == 1 && m[af.root.getName()])
								w.call(this, F, af, P);
						}
				...
				function B(F) {
					return function (G) {
						...
						if (K && (K.name && K.name == 'br' || K.value && (L = K.value.match(z)))) {
							var M = K;
							if (!(L && L.index) && M == H[0])
								H[0] = F || c ? new a.htmlParser.text('\xa0') : new a.htmlParser.element('br', {});
							else if (M.name == 'br')
								H.splice(I - 1, 1);
							else
								M.value = M.value.replace(z, '');
						}
					};
				}
				;
				var C = {elements: {}};
				for (var D in y.$listItem)
					C.elements[D] = B();
				var E = {elements: {}};
				for (D in y.$listItem)
					E.elements[D] = B(true);
			...
			(function () {
				var m = {
					modes: {wysiwyg: 1, source: 1}, canUndo: false, exec: function (o) {
						...
						if (q.fullPage)
							p = o.getData().replace(/<head>/, '$&' + r).replace(/[^>]*(?=<\/title>)/, o.lang.preview);
						else {
					...
					requires: ['menubutton'], beforeInit: function (t) {
						...
						if (t.contextMenu && t.addMenuItems)
							t.contextMenu.addListener(function (G, H) {
								if (!q.isScaytEnabled(t) || H.getCommonAncestor().isReadOnly())
									return null;
					...
					requires: ['richcombo', 'styles'], init: function (n) {
						...
						n.ui.addRichCombo('Styles', {
							...
							onClick: function (t) {
								...
								if (u.type == 2 && u.checkActive(w))
									u.remove(n.document);
								else if (u.type == 3 && u.checkActive(w))
									u.remove(n.document);
								else
									u.apply(n.document);
								n.fire('saveSnapshot');
			...
			(function () {
				...
				function q(E) {
					if (E.getType() == 3)
						return E.getSelectedElement().isReadOnly();
					else
						return E.getCommonAncestor().isReadOnly();
				}
				;
				function r(E) {
					return function (F) {
				...
				function s(E) {
					...
					if (c) {
						...
						K.document.$.execCommand('inserthtml', false, E);
					if (b.webkit) {
						F = K.getSelection();
						F.scrollIntoView();
					}
				...
				function C(E) {
					...
					if (!I || I == 1 && H.getFirst().hasAttribute('_moz_editor_bogus_node')) {
						...
						G.$.dispatchEvent(N);
						if (L != K.$.scrollTop || M != K.$.scrollLeft)
							J.getWindow().$.scrollTo(M, L);
						I && H.getFirst().remove();
				...
				j.add('wysiwygarea', {
					requires: ['editingblock'], init: function (E) {
						...
						E.on('editingBlockReady', function () {
								...
								if (c) {
									...
									setTimeout(function () {
										if (b.gecko && b.version >= 10900 || b.opera)
											...
											Y.$.body.parentNode.contentEditable = true;
										else
											Y.$.designMode = 'on';
									}, 0);
								...
								if (!(c || b.opera))
									Y.on('mousedown', function (af) {
										...
										if (ag.is('img', 'hr', 'input', 'textarea', 'select'))
											E.getSelection().selectElement(ag);
									});
								if (b.gecko)
							...
							E.addMode('wysiwyg', {
								load: function (X, Y, Z) {
									...
									if (E.dataProcessor)
										ab = E.dataProcessor.toDataFormat(ab, F);
						...
						if (b.ie8Compat) {
							...
							var J = [];
							for (var K in f.$removeEmpty)
								J.push('html.CSS1Compat ' + K + '[contenteditable=false]');
			...
			(function () {
				...
				function z(P) {
					...
					P.on('load', function () {
						var ac = '';
						if (R == 1)
							ac = ' cke_resizer_horizontal';
						else if (R == 2)
							ac = ' cke_resizer_vertical';
					...
					function aa(ac) {
						...
						if (al)
							al = al == 'auto' ? V.width - (am.x || 0) - ak.getSize('width') : parseInt(al, 10);
						if (am.y + aj > V.height)
							aj = V.height - am.y;
						if ((ad ? al : am.x) + ai > V.width)
							ai = V.width - (ad ? al : am.x);
						if ((R == 1 || R == 3) && !(ad && ae > 0 && !am.x))
							ag = Math.max(Q.minWidth || 0, ai - T);
						if (R == 2 || R == 3)
							ah = Math.max(Q.minHeight || 0, aj - U);
						P.resize(ag, ah);
						Y(al);
					...
					function ab() {
						...
						if (S.lang.dir == 'rtl') {
							...
							if (ae == 'auto')
								ae = V.width - parseInt(ad.getStyle('right'), 10) - P.getSize().width;
							else
								ae = parseInt(ae, 10);
			...
			e.extend(a.editor.prototype, {
				openDialog: function (m, n) {
					...
					if (typeof o == 'function' && p._isLoaded) {
						...
						throw new Error('[CKEDITOR.dialog.openDialog] Dialog "' + m + '" failed when loading definition.');
					...
					function v(x) {
						...
						if (!z._isLoaded || w && typeof x == 'undefined')
							return;
						if (typeof y != 'function')
							a.dialog._.dialogDefinitions[m] = 'failed';
						u.openDialog(m, n);
						s.setStyle('cursor', t);
					}
					;
			...
			(function () {
						...
						switch (V.type) {
							...
							case 2:
								...
								for (var T = 0, U; T < S.length; T++) {
									U = S[T];
									if (V.type == 2 && (U == R.block || U == R.blockLimit))
										continue;
									if (V.type == 3 && !(U.getName() in n))
										continue;
									if (V.checkElementRemovable(U, true))
										return true;
								}
						}
						return false;
				...
				function p(R) {
					...
					while (R = R.getParent()) {
						if (R.getName() == 'body')
							break;
						if (R.getAttribute('data-cke-nostyle'))
							S = R;
						else if (!T) {
							...
							if (!ai || Y[ai] && !ak && (!aj || X) && (ac.getPosition(ab) | 4 | 0 | 8) == 4 + 0 + 8 && (!V.childRule || V.childRule(ac))) {
								...
								if (al && ((al.getDtd() || f.span)[U] || W) && (!V.parentRule || V.parentRule(al))) {
									...
									if (ah == 3 || aj || ah == 1 && !ac.getChildCount()) {
										var am = ac, an;
										while (!am.$.nextSibling && (an = am.getParent(), Y[an.getName()]) && (an.getPosition(aa) | 2 | 0 | 8) == 2 + 0 + 8 && (!V.childRule || V.childRule(an)))
											am = an;
										ad.setEndAfter(am);
										if (!am.$.nextSibling)
											ag = true;
							...
							while (ao && aq) {
								if (aq.getName() == U) {
									for (as in V.attributes) {
										if (ar.blockedAttrs[as] || !(au = aq.getAttribute(at)))
											continue;
										if (ao.getAttribute(as) == au)
											ar.attrs[as] = 1;
										else
											ar.blockedAttrs[as] = 1;
									}
									for (at in V.styles) {
										if (ar.blockedStyles[at] || !(au = aq.getStyle(at)))
											continue;
										if (ao.getStyle(at) == au)
											ar.styles[at] = 1;
										else
											ar.blockedStyles[at] = 1;
									}
								}
								aq = aq.getParent();
							}
							for (as in ar.attrs)
								ao.removeAttribute(as);
							for (at in ar.styles)
								ao.removeStyle(at);
							if (ap && !ao.hasAttributes())
								ao = null;
							if (ao) {
					...
					if (R.collapsed) {
						...
						function ad() {
							...
							for (var ak = 0; ak < ag.elements.length; ak++) {
								...
								if (al == ag.block || al == ag.blockLimit)
									break;
								if (ac.checkElementRemovable(al))
									ai = al;
							}
							for (ak = 0; ak < ah.elements.length; ak++) {
								al = ah.elements[ak];
								if (al == ah.block || al == ah.blockLimit)
									break;
								if (ac.checkElementRemovable(al))
									aj = al;
							}
				...
				function t(R) {
					...
					if (W)
						for (var Y in W)
							T.removeAttribute(Y, W[Y]);
					if (V.styles)
						for (var Z in V.styles) {
							if (!V.styles.hasOwnProperty(Z))
								continue;
							T.removeStyle(Z);
						}
				}
				...
				function u(R) {
					...
					if (this._.enterMode)
						T.enlargeBr = this._.enterMode != 2;
				...
				function x(R) {
					var S;
					if (!((S = R.getPrevious(w)) && S.is && S.is('pre')))
						return;
					var T = z(S.getHtml(), /\n$/, '') + '\n\n' + z(R.getHtml(), /^\n/, '');
					if (c)
						R.$.outerHTML = '<pre>' + T + '</pre>';
					else
						R.setHtml(T);
					S.remove();
				}
				...
				function C(R, S) {
					...
					for (var X in U) {
						if ((X == 'class' || R._.definition.fullMatch) && S.getAttribute(X) != M(X, U[X]))
							continue;
						W = S.hasAttribute(X);
						S.removeAttribute(X);
					}
					for (var Y in V) {
						if (R._.definition.fullMatch && S.getStyle(Y) != M(Y, V[Y], true))
							continue;
						W = W || !!S.getStyle(Y);
						S.removeStyle(Y);
					}
					...
					if (T)
						for (var U = 0; U < T.length; U++) {
							...
							if (W = R.getAttribute(V)) {
								var X = T[U][1];
								if (X === null || X.test && X.test(W) || typeof X == 'string' && W == X)
									R.removeAttribute(V);
							}
						}
					F(R);
				...
				function H(R, S) {
					...
					if (U)
						for (var W in U)
							R.setAttribute(W, U[W]);
					if (V)
						R.setAttribute('style', V);
					return R;
				}
				;
				var I = /#\((.+?)\)/g;
				...
				function J(R, S) {
					for (var T in R)
						R[T] = R[T].replace(I, function (U, V) {
							return S[V];
						});
				}
				;
				...
				function P(R, S) {
					...
					for (var T in R) {
						if (!(T in S && (S[T] == R[T] || R[T] == 'inherit' || S[T] == 'inherit')))
							return false;
					}
					return true;
				}
				...
				m.prototype = {
					getNextParagraph: function (p) {
						...
						if (!q) {
							...
							q = J.block;
							if (!q && !this.enforceRealBlocks && L[K.getName()] && r.checkStartOfBlock() && r.checkEndOfBlock())
								q = K;
								...
								this._.nextNode = q.equals(y) ? null : r.getBoundaryNodes().endNode.getNextSourceNode(true, null, y);
						}
						...
						if (v) {
							...
							if (P && P.type == 1 && P.getName() == 'br')
								if (c || P.getPrevious(N) || P.getNext(N))
									P.remove();
						}
						if (!this._.nextNode)
							this._.nextNode = s || q.equals(y) ? null : q.getNextSourceNode(true, null, y);
						if (!N(this._.nextNode))
							this._.nextNode = this._.nextNode.getNextSourceNode(true, null, function (Q) {
					...
					getNative: c ? function () {
						...
						var t = c ? (function () {
							var u = function (v, w) {
								...
								try {
									while (E > 0)
										E -= y[--A].nodeValue.length;
								} catch (F) {
									E = 0;
								}
								if (E === 0)
									return {container: x, offset: A};
								else
									return {container: y[A], offset: -E};
							};
							return function () {
								var F = this;
								...
								if (x == 2) {
									...
									if (y.endContainer.getPosition(y.startContainer) & 4 && y.endOffset <= y.startContainer.getIndex())
										y.collapse();
									return [y];
								} else if (x == 3) {
									var A = [];
						...
						if (c) {
							...
							for (var w = 0; w < t.length; w++) {
								...
								if (A.collapsed && (b.opera || b.gecko && b.version < 10900) && C.type == 1 && !C.getChildCount())
									C.appendText('');
