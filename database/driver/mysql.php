...
class Driver_mysql extends Database {
    ...
    public function query($sql=null) {
        ...
        if (($this->cursor = $this->result = mysql_query($this->sql, $this->conn)) === false) {
            throw new Exception(mysql_error($this->conn)." SQL = $this->sql");
        }

        if (! $this->cursor) {
            $this->errorNum = mysql_errno($this->conn);
            $this->errorMsg = mysql_error($this->conn)." SQL = $this->sql";
            return false;
        }
