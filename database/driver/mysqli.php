...
class Driver_mysqli extends Database
{
    ...
    protected function connect()
    {
        ...
        if (empty($this->options['db_port'])) {
            /* If empty db_port, try to set it with the mysqli.default_port with the PHP @ini_get function */
            $this->options['db_port'] = @ini_get("mysqli.default_port");
            /* After that try set up the port for the next connection to the mysqli */
            /* unless the the port is provided into de 'database' config file */
    ...
    public function fetchAssocRow()
    {
        ...
        $ret = null;
        if (null != $array = mysqli_fetch_assoc($this->result)) {
            $ret = $array;
        }
        mysqli_free_result($this->result);
        return $ret;
    ...
    public function fetchObjectRow($className = 'stdClass')
    {
        try {
            $this->execute();
            $ret = null;
            if (null != $object = mysqli_fetch_object($this->result, $className)) {
                $ret = $object;
            }
            mysqli_free_result($this->result);
            return $ret;
    ...
    public function fetchRow()
    {
        ...
        $ret = null;
        if (null != $row = mysqli_fetch_row($this->result)) {
            $ret = $row;
        }
        mysqli_free_result($this->result);
        return $ret;
