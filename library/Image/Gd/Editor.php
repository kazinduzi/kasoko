...
class Gd_Editor extends Editor
{
    ...
    {
        if (!self::test()) {
            throw new \Exception('GD library is not available or ...');
        }
        $this->file = $file;
    }
    ...
    public function load()
    {
        ...
        if (!is_resource($this->image)) {
            throw new \Exception('File #' . $this->file . 'is not an image.');
        }
        $size = @getimagesize($this->file);
        if (!$size) {
            throw new \Exception('Could not read image size of the file #' . $this->file);
        }
    ...
    public function resize($max_w, $max_h, $crop = false)
    {
        ...
        throw new \Exception('Image resize failed of the file #' . $this->file);
    }
    ...
    protected function _resize($max_w, $max_h, $crop = false)
    {
        ...
        if (!$dims) {
            throw new \Exception('Could not calculate resized image dimensions' . $this->file);
        }
        ...
        throw new \Exception('Image resize failed.' . $this->file);
    }
    ...
    public function rotate($angle, $background = null)
    {
        ...
        throw new \Exception('Image rotate failed: File:' . $this->file);
    }
    ...
    public function flip($horz, $vert)
    {
        ...
        throw new \Exception('Image flip failed. File:' . $this->file);
    }
    ...
    private function image_resize_dimensions($orig_w, $orig_h, $dest_w, $dest_h, $crop = false)
    {
        ...
        if ($new_w >= $orig_w && $new_h >= $orig_h) {
            return array(0, 0, (int) $s_x, (int) $s_y, (int) $orig_w, (int) $orig_h, (int) $crop_w, (int) $crop_h);
        }
